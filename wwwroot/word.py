...
def change(req, wid = None):
	...
	for attribute in edfield_results.getresult():
		if attribute[0] == 1: # string attribute
			...
			else: oldval = str(vresults.getresult()[0][0], 'UTF-8')
			...
			messages.append("%s: '%s' -> '%s'" % (str(attribute[2], 'UTF-8'),
			                oldval, newval))
	...
	if event_inserted and len(messages) > 0:
		mess_str = jotools.escape_sql_string(reduce(lambda x, y: x + "\n" + y, messages, ""))
...
def flags(req, wid = None):
	...
	for attribute in edfield_results.getresult():
		...
		if newval == False:
			...
			messages.append(_("Flag removed: '%s'") % str(attribute[1], 'UTF-8'))
		if newval == True:
			db.query(("insert into flag_attribute_value(wid, aid, eevent) " +
			          "values(%i, %i, %i)") % (wid_n, attribute[0], eid))
			messages.append(_("Flag added: '%s'") % str(attribute[1], 'UTF-8'))
...
def rwords(req, wid = None):
	...
	for attribute in rword_res:
		...
		db.query("delete from related_word where wid = %i and rwid = %i" \
		         % (wid_n, attribute[0]))
		messages.append(_("Alternative spelling removed: '%s'") \
		                % jotools.escape_html(str(attribute[1], 'UTF-8')))
